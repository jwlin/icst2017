...
"""
Infer testing data with rules from training data
"""
...
if __name__ == '__main__':
    ...
    input_dir = os.path.join(current_dir, 'corpus', 'all-input-wo-attr-name')
    ...
    for dname in dnames:  # e.g. 10-training, 20-training, ...
        ...
        for sub_dname in sub_dnames:  # e.g. trial-20160316-010033, trial-20160316-010035, ...
            ...
            for k, v in rules.items():
                ...
                for k2, v2 in v.items():
                    subnum_str += len(v2)
                rule_match_logs.append('Topic: %s, Rules: %s (%d)\n' % (k, v, subnum_str))
                num_str += subnum_str
            rule_match_logs.append('# of rules: %d topics, %d strings\n' % (len(rules.keys()), num_str))
            num_topics.append(len(rules.keys()))
            num_rule_string.append(num_str)
        ...
        with open(os.path.join(trial_dir, dname, 'program_rule_match.log'), 'w') as f:
            ...
            arr = numpy.array(num_topics)
            f.write('avg topics: %f\t' % numpy.mean(arr))
            f.write('std: %f\t' % numpy.std(arr))
            f.write('min: %f\t' % numpy.min(arr))
            f.write('max: %f\n' % numpy.max(arr))

            arr = numpy.array(num_rule_string)
            f.write('avg strings built for rule matching: %f\t' % numpy.mean(arr))
            f.write('std: %f\t' % numpy.std(arr))
            f.write('min: %f\t' % numpy.min(arr))
            f.write('max: %f\n' % numpy.max(arr))
        #raw_input(dname)
